...
群聊中发生提示词注入 → 模型自动调用 `report_abuse` → 管理群收到举报请示（正文 + 合并转发聊天记录）→ 引用请示回复「同意」批准拉黑 → 被拉黑用户的所有消息（含命令）从此被完全拦截：
...
**Q：举报请示的隐私如何处理？**
聊天记录缓存仅存内存（重启清空），举报记录与文本快照保存在你自己的 AstrBot 数据目录，不会上传任何第三方；请示附带的聊天记录可能包含该用户此前与 bot 的其他会话内容，请知悉。发送到管理群时请示包含被举报人完整 QQ 号，请确保管理群成员可信。
